...
## Vídeo da revelação (PENDENTE — em produção)
Proporção: **9:16 vertical**. Quando o mp4/link final existir, ligar no funil (`VIDEO_DATA.enabled=true`, `url`, ajustar slot pra 9:16 no app.js `showProtocol`). Placeholder está ESCONDIDO no ar (else branch vazio) até o vídeo existir.
